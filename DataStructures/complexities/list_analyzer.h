...
namespace ds::utils
{
     ...
     */
    template<class List>
    class ListAnalyzer : public ComplexityAnalyzer<List>            //analyzuje list / vektor arrayList linkedList etc.
    {
    protected:
    ...
    public:
            ...
            CompositeAnalyzer("Lists")
        {
            ...
            this->addAnalyzer(std::make_unique<ListRemoveAnalyzer<std::list<int>>>("list-remove"));    //tu si REGISTRUJEME TESTY
        }
    };


    //                             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^     //std::function<void(List&, size_t
       // TODO 01 Namiesto defaultne vytvoreného - prázdneho - funkèného objektu je potrebné
       //         parameter správne inicializova!
    template<class List>
    ListAnalyzer<List>::ListAnalyzer(const std::string& name) :                 //ako sa do zoznamu vklada to riesi
        ComplexityAnalyzer<List>(name,[this](List& structure, size_t n)
            {
    ...
    {
        structure.erase(structure.begin());      //iterator ukazovatko   begin ukazuje na 1 prvok - index nemozem lebo list
    }
}
