...
namespace ds::utils
{
    ...
    public:
        ...
        const std::vector<std::unique_ptr<Analyzer>>& getAnalyzers(); //  to cele to vracia = navratova hodnota std::vector<std::unique_ptr<Analyzer>>&  
                                                                      //  a nazov funkcie je getAnalyzers()
    ...
    {
        this->resetSuccess();
        std::map<size_t, std::vector<duration_t>> data;     //vytvori mapu klucov kde zapise expectedVelkost a zapise ku nim kolko dlho to trvalo
            for (int rep = 0; rep < this->getReplicationCount(); rep++)
            {
                Structure structure(structurePrototype);

                this->beforeReplication(structure);

                for (int step = 1; step <= this->getStepCount(); step++)
                {
                    size_t expectedSize = step * this->getStepSize();
                    size_t insertCount = step * this->getStepSize() - structure.size();
                    this->insertN_(structure, insertCount);
