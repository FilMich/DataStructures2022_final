...
namespace ds::adt {
    ...
    public:
        void insert(K key, T data) override;
        void insertDup(K key, T data);
        bool tryFindDup(K key, ImplicitList<T>*& list);
        T remove(K key) override;
    ...
    public:
        ...
        void insert(K key, T data) override;
        void insertDupp(K key, T data);
        bool tryFind(K key, T*& data) override;
        bool tryFindDupp(K key, ImplicitList<T>*& list);
        T remove(K key) override;
    ...
    void UnsortedImplicitSequenceTable<K, T>::insert(K key, T data)
    {
        ...
        {
            this->error("UnsortedImplicitSequenceTable<K, T>::insert(K key, T data): Key exists in table");
        }
    ...
    T UnsortedImplicitSequenceTable<K, T>::remove(K key)
    {
        ...
        if (block == nullptr)
        {
            this->error("UnsortedImplicitSequenceTable<K, T>::remove(K key): key is not in the table");
        }
        else
        {
    ...
    void UnsortedExplicitSequenceTable<K, T>::insert(K key, T data)
    {
        if (this->contains(key))
        {
            this->error("UnsortedExplicitSequenceTable<K, T>::insert(K key, T data): Key exists in table");
        }
        this->getSequence()->insertLast().data_ = { key,data };
    }
    ...
    template<typename K, typename T>
    inline void SortedSequenceTable<K, T>::insertDup(K key, T data)
    {
        ...
        {
            tableItem = &this->getSequence()->insertFirst().data_;
            tableItem->list_ = new ImplicitList<T>(); //dddddddddddddd
            tableItem->key_ = key;
            tableItem->data_ = data;
            tableItem->list_->insertLast(data);
        }
        else
        {
            BlockType* blok = nullptr;
            if (this->tryFindBlockWithKey(key, 0, this->size(), blok))
            {
                (*blok).data_.list_->insertLast(data);
                /*for (auto a : (*blok).data_.list_) 
                { 
            ...
            else
            {
                tableItem = key > blok->data_.key_
                    ? &this->getSequence()->insertAfter(*blok).data_
                    : &this->getSequence()->insertBefore(*blok).data_;
                tableItem->list_ = new ImplicitList<T>();
                tableItem->key_ = key;
                tableItem->data_ = data;
                tableItem->list_->insertLast(data);
            }
    ...
    template<typename K, typename T>
    inline bool SortedSequenceTable<K, T>::tryFindDup(K key, ImplicitList<T>*& list)
    {
        BlockType* block = this->findBlockWithKey(key);
        ...
        {
            list->assign(*block->data_.list_);
            //list = block->data_.list_;
    ...
    template<typename K, typename T, typename BlockType>
    void GeneralBinarySearchTree<K, T, BlockType>::insertDupp(K key, T data)
    {
        ...
        {
            newNode = &this->getHierarchy()->emplaceRoot();
            newNode->data_.list_ = new ImplicitList<T>();
            newNode->data_.list_->insertLast(data);
            newNode->data_.key_ = key;
            newNode->data_.data_ = data;
            size_++;
            this->balanceTree(newNode);
        ...
        else
        {
            ...
            if (nodeInRelation->data_.key_ == key)
            {
                (*nodeInRelation).data_.list_->insertLast(data);
            }
            else
            {
                newNode = &this->insertNode(key, nodeInRelation);
    ...
    template<typename K, typename T, typename BlockType>
    bool GeneralBinarySearchTree<K, T, BlockType>::tryFindDupp(K key, ImplicitList<T>*& list)
    {
        BVSNodeType* nodeWithKey = nullptr;
        if (!this->tryFindNodeWithKey(key, nodeWithKey))
        {
        ...
        list->assign(*nodeWithKey->data_.list_);
        //data = &nodeWithKey->data_.data_;
        return true;
    }
