...
namespace ds::adt {
    ...
    void BinaryHeap<P, T>::push(P priority, T data)
    {
        ...
        /*HierarchyBlockType* curr = &this->getHierarchy()->insertLastLeaf();
        curr->data_ = { priority, data };
        HierarchyBlockType* parent = this->getHierarchy()->accessParent(*curr);

        while (parent != nullptr && curr->data_.priority_ < parent->data_.priority_) 
        {
            std::swap(curr->data_, parent->data_);
            curr = parent;
            parent = this->getHierarchy()->accessParent(*curr)''
        }*/
    ...
    T& BinaryHeap<P, T>::peek()
    {
        if (this->isEmpty())
        {
            this->error("BinaryHeap<P, T>::peek(): PQ is empty");
        }
