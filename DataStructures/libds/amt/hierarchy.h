...
namespace ds::amt {
    ...
    bool Hierarchy<BlockType>::isNthSon(const BlockType& node, size_t sonOrder) const
	{
		...
		if (parent != nullptr)
		{
			return this->accessSon(*parent, sonOrder) == &node;
		}
		else
		{
			return false;
	...
	{
		if (!this->currentPosition_->currentNodeProcessed_) {    //bol currentNodeProcessed_ procesovany uz alebo nie?
			//ak nie tak skusime ist na laveho syna
			if (this->currentPosition_->currentSonOrder_ != BinaryHierarchy<BlockType>::LEFT_SON_INDEX && tryToGoToLeftSonInCurrentPosition()) {
				//size_t currentSonOrder_; 
				this->savePosition(this->currentPosition_->currentSon_);
				this->operator++(); // rekurzivne call operator++()
			}
		}
		else {
			if (this->currentPosition_->currentSonOrder_ != BinaryHierarchy<BlockType>::RIGHT_SON_INDEX && tryToGoToRightSonInCurrentPosition()) {
				this->savePosition(this->currentPosition_->currentSon_);
				this->operator++(); // rekurzivne call operator++()
			}
			else {
		...
		{
			if (this->currentPosition_->currentSonOrder_ != LEFT_SON_INDEX && tryToGoToRightSonInCurrentPosition())
			{
				this->savePosition(this->currentPosition_->currentSon_);
				++(*this);
			}
